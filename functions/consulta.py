...
driver_version = environ.get('CHROMEDRIVER')
driver_path = environ.get('PATHCHROMEDRIVER')
...
def consulta(nomeC,cel,email):
    ...
    # driver = webdriver.Chrome(service=Service(ChromeDriverManager(driver_version=driver_version).install()))
    driver.get(url)
    sleep(7)
